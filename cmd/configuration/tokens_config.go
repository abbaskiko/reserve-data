...
var TokenConfigs = map[deployment.Deployment]map[string]common.Token{
...
func mustGetTokenConfig(dpl deployment.Deployment) map[string]common.Token {
	result, avail := TokenConfigs[dpl]
	if avail {
		return result
	}
	...
	log.Panicf("cannot get token config for mode %s", dpl.String())
	return nil
}
