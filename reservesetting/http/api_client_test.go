...
func (c *apiClient) updateExchangeStatus(id uint64) (commonResponse, error) {
	req, err := createRequest(http.MethodPut, fmt.Sprintf("/v3/update-exchange-status/%d", id), nil)
	if err != nil {
		return commonResponse{}, err
	}
	resp := httptest.NewRecorder()
